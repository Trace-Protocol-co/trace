...
const x402Route = {
    ...
    description: "TRACE media-authenticity verification — returns a verdict (VERIFIED_ORIGINAL / MODIFIED / AI_GENERATED / UNVERIFIED) with confidence and on-chain provenance. Provide `url` (JSON) or a `file` upload (multipart).",
    mimeType: "application/json",
};
app.use(paymentMiddleware({ "GET /agent/verify": x402Route, "POST /agent/verify": x402Route }, x402Server, undefined, // paywallConfig
undefined, // paywall
false));
...
if (process.env.OKX_API_KEY && process.env.OKX_SECRET_KEY && process.env.OKX_PASSPHRASE) {
    x402Server
        ...
        .catch((err) => console.error("[x402] facilitator init failed; /agent/verify will error until OKX credentials are valid:", err instanceof Error ? err.message : err));
}
else {
